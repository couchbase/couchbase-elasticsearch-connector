...
A type definition is a rule for matching a document ID, and instructions for what to do with matching documents.

The order of type definitions is significant.
If a document matches more than one type, the type definition that appears first in the config file takes precedence.
...
In addition to the fields that can have default values, every type definition must have a rule for matching document IDs.
