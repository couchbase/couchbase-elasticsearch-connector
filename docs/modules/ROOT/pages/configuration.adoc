...
----
<1> (Since 4.2.9; Uncommitted) If true, document lifecycle milestones will be logged at INFO level instead of DEBUG.
...
<2> (Since 4.2.9; Uncommitted) Determines which kinds of sensitive log messages will be tagged for later redaction by the Couchbase log redaction tool.
...
WARNING: Support for client certificate authentication, also known as "mutual TLS authentication" (mTLS), was introduced in version 4.2.8 of the connector as an "uncommitted" feature; the configuration options described in this section may change without notice.
...
<5> (Since 4.2.3; Uncommitted) The name of the collection for storing metadata like replication checkpoint documents.
...
<9> (Since 4.2.6; Uncommitted) Set this to `false` if you are using a secure connection to Couchbase but for whatever reason need to disable TLS hostname verification.
...
<10> (Since 4.2.3; Uncommitted) If you want to replicate from only one scope, name it here.
<11> (Since 4.2.3; Uncommitted) If you want to replicate from a subset of collections within a scope, or collections in different scopes, name the collections here.
...
----
<1> (Since 4.2.10; Uncommitted) Any property name recognized by the Couchbase Java SDK may be specified in this section.
...
If connecting directly to an instance of the Amazon Elasticsearch Service, all Elasticsearch requests must be signed with AWS credentials.
...
Leave this blank unless you're connecting directly to an instance of the Amazon Elasticsearch Service.
...
<6> (Since 4.2.3; Uncommitted) If `true`, the `prefix` and `regex` properties described in the next section match against the qualified document ID, which includes the document's scope and collection.
...
Since version 4.2.11, integer and boolean values may be specified as strings.
For example: `age = '$\{AGE}'`.
