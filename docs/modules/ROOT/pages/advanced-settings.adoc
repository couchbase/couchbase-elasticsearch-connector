 ...
 ** `org.elasticsearch.transport.couchbase.capi.DelimiterTypeSelector`: If the document ID is in the format `<__type__><__delimiter__><*>`, documents are mapped to the type specified by `<__type__>`.
