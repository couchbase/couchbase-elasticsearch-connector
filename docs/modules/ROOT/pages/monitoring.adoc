...
Since 4.2.6, metrics are also exposed in Prometheus format at `http://localhost:31415/metrics/prometheus`.
In connector version 4.2.x, all Prometheus metrics are "Uncommitted API" which means they may change between releases without notice.
...
`cbes.backlog` (Since 4.2.5; uncommitted)::
...
_All the "backfill*" metrics are deprecated since they do not provide an accurate view of the connector's progress; please use the `cbes.backlog` metric instead._

`cbes.backfillRemaining` (DEPRECATED)::
...
`cbes.backfillEstTimeLeft` (DEPRECATED)::
A rough estimate of the time in seconds it will take for the connector to finish processing the remaining backfill.
