...
A more sophisticated check should parse the JSON response and inspecting the values of whichever metrics you consider important.
...
Setting the value to `-1` will disable the HTTP server.
...
The location of the metrics log file may be configured by editing `$CBES_HOME/config/log4j2.xml`.
...
An exceptionally long duration may indicate the connector is stalled in a retry loop for some reason.

`cbes.backfill`::
The number of Couchbase documents not yet replicated to Elasticsearch at the time the connector started.
...
`cbes.writeQueue`::
Reports the number of document change events currently buffered in memory. (The write queue is implicitly bounded by the `flowControlBuffer` config property which determines the buffer size.)
...
A timer combines a meter with a histogram of event duration, providing insight into the percentiles.
The histogram is backed by an exponentially decaying reservoir, representing roughly the past 5 minutes of data.

`cbes.latency`::
The time between when the connector is notified of a database change event and when the change is written to Elasticsearch.
...
The connector exposes several other metrics that may be useful for troubleshooting.
