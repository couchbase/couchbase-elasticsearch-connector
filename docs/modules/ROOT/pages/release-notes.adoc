...
== Standalone Connector vs. Plugin

Starting with version 4.0, the connector is now a standalone service.
The service distribution archives are named `couchbase-elasticsearch-connector-`.

Prior to version 4.0, the connector was implemented as an Elasticsearch plugin.
The plugin distributions have archive names starting with `elasticsearch-transport-couchbase-`.
...
* The connector is a separate process instead of an Elasticsearch plugin.
